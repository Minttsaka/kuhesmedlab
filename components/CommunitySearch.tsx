...
import { Post, Support } from '@prisma/client'
...
export default function CommunitySearch({supportList, searchQuery }:{searchQuery:string | string[] | undefined, supportList:Post[]}) {
  ...
  const semanticSearch = useCallback((query:string | string[] | undefined) => {
    ...
    return supportList.map(support => {
      const concepts = support.title.split(" ")
      const titleTokens = removeStopWords(tokenize(support.title)).map(stemWord)
      const descriptionTokens = removeStopWords(tokenize(support.body!)).map(stemWord)
              ...
              <li key={result.id}>
                <Card>
                  <CardContent className="pt-6">
                    <Link href={`/community/post/${result.slug}`} className="font-medium hover:underline text-lg mb-2">
